"""Kaggriculture agent — MapleLeaf 4.5
...
Market:  price-impact SELL sort + NPC-demand persistence weighting
         ...
         + NPC-threat-weighted opponent exposure (log-scale yield)
Safety:  shed-projection clamp so SELL quantities never exceed actual inventory

MapleLeaf 4.5 vs 4.3 changes (fixing all documented-but-unwired features):
  1. Wire all 6 missing pipeline functions: _expand_route_sells, _merge_sells,
     _overflow_sells, _wheat_buffer_sell, _price_gate_sells,
     _detect_opponent_sells (were defined in 4.3 but never called).
  2. Fix _clone_threshold: add day>=20 → 6 third branch (was missing in 4.3).
  3. Fix _PREMIUM_ITEMS: add EGG and FERTILIZER (6 items, was 4 in 4.3).
  4. Fix _premium_shift: implement actual 2-step lookahead with step+2 tranche.
  5. Per-item price gate thresholds (NPC-recovery-aware: WHEAT 0.60, MELON 0.15).
  6. Opponent dump integration: hold high-NPC sells 1 turn after opponent dumps.
  7. Log-scale opponent exposure: log1p(yield_units) matches market concavity.
  8. Lower shed overflow threshold: 92→75 (earlier force-sell prevents lost drops).
  9. Pre-terminal no-recovery sells: MELON/WOOL/FERTILIZER bleed from step -7.
"""
import base64
import copy
import json
...
_NO_RECOVERY_ITEMS = frozenset(("MELON", "WOOL", "FERTILIZER"))


def _preterminal_no_recovery(obs, action):
    """7 steps before end: sell no-recovery items before opponent dumps them.

    MELON (sq above, 3.6×), WOOL (sq above, 3.2×), FERTILIZER (zero NPC) will
    never recover price after a flood.  Bleeding them out early, before both
    players pile in at the final 3 steps, nets meaningfully higher per-unit prices.
    """
    action  = _align_hands(action, obs)
    shed    = _projected_shed(obs, action)
    market  = list(action.get("market") or [])
...
# Per-item price gate thresholds — calibrated to NPC recovery speed.
# High-NPC items (WHEAT: log/0.2, EGG: log/0.2) recover quickly → high gate.
# Low-NPC items (MELON: sq/3.6, WOOL: sq/3.2) barely recover → low/no gate.
_PRICE_GATE_THRESH = {
    "WHEAT":       0.60,
    "EGG":         0.55,
    "CARROT":      0.50,
...
def _price_gate_sells(obs, action, opp_sold=None):
    """Skip SELL orders where price has crashed below the per-item recovery floor.

    High-NPC items (WHEAT 60%, EGG 55%) recover fast — a dip is temporary.
    Low-NPC items (MELON 15%, FERTILIZER 15%) barely recover — gate is near-zero.

    When opp_sold is provided (items opponent just dumped), additionally hold
    high-NPC sells for 1 turn so we sell into the recovered price, not the dump.
    """
    action = _copy_action(action)
    day = int(_get(obs, "day", 0) or 0)
    ...
    for raw in list(action.get("market", []) or []):
        ...
        if len(order) >= 3 and order[0] == "SELL" and order[1] in _BASE_PRICES:
            ...
            thresh    = _PRICE_GATE_THRESH.get(item, 0.20)
            if cur_price < _BASE_PRICES[item] * thresh:
                continue  # price below recovery floor; NPC demand will restore it
            # Hold high-NPC sells for 1 turn if opponent just dumped that item
            if opp_sold and item in opp_sold:
                npc = _npc_eff(item, day, obs)
                if npc >= 2.0:  # meaningful recovery in 2-3 turns
                    continue
        market.append(order)
    action["market"] = market
    return action
...
_PREMIUM_MAX_QTY = 30
_SHED_OVERFLOW   = 75   # early force-sell prevents discarded end-of-day drops
...
def _clone_threshold(obs):
    ...
    day = int(_get(obs, "day", 0) or 0)
    return 12 if day < 10 else (8 if day < 20 else 6)
