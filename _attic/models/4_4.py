"""Kaggriculture agent — MapleLeaf 4.4
...
vs 4.3: wired _merge_sells (fixed order-preserving bug), _detect_opponent_sells,
        _preterminal_no_recovery; 2-step premium_shift; log1p opponent exposure.
        Dropped: _expand_route_sells (crashes market), _price_gate_sells (drops
        route sells with no reschedule, strands items in shed).
"""
import base64
import copy
import json
...
def agent(obs):
    try:
        ...
        _detect_opponent_sells(obs, step)   # keep market-inv tracker current
