"""MapleLeaf 5.5 — pure route replay with minimal safety overlays.

Executes THUNDER THUNDER's best-known routes exactly as recorded, with only
two lightweight safety wrappers applied on top:

  1. Weed repair     — if a tile turns into WEED before a BUILD/PLANT, issue DIG
                       first and replay the intended action up to 8 steps later.
                       Prevents the farm from permanently losing a tile.
  2. Terminal sell   — at step 716+, sell any items remaining in the shed that
                       the route didn't explicitly schedule. Captures late
                       inventory that accumulated after the route's last sells.

Everything else from 5.3/5.4 is removed:
  - No rank_sell_slots  (no price-impact reordering)
  - No preempt_shift    (no clone-detection front-running)
  - No repay_shift
  - No price_floor_guard
  - No wheat-arbitrage counter
  - No opponent cash exploit
  - No wheat bleed
  - No market params or demand calculations

Routes:
  P0 backbone: ep 91385999 (THUNDER THUNDER as P0, 139,403 pts)
  P1 backbone: ep 91471546 (THUNDER THUNDER as P1, 148,866 pts)
"""
import base64
import copy
import json
...
__version__ = "mapleleaf-5.5-route-only"
