"""MapleLeaf 6.4 -- Nikita Lugovoy 8-COW/4-SHEEP route + widened premium
market-lead for Kaggriculture.

Context: 6.3's rating climbed from ~1780 (at submission) to ~2781 over its
first ~13 hours live, so the premise that it urgently needed fixing was
wrong -- but a forensic pass on fresh real losses at that (much tougher)
rating tier found something real: our route's total premium output
(MILK+WOOL+STRAWBERRY+MELON) was ~773 units almost every single game (our
backbone is largely deterministic), while several real opponents outsold
us by 20-55% in the same categories. Two follow-up findings ruled out the
obvious fixes: land is not the constraint (both sides leave >70% of
unlocked tiles empty at game end), and "just add more animals" is a net
loss once the game's real labor economics are modeled -- hands are day
laborers, wiped every single day (`farm["hands"] = []` at each day
boundary) and must be re-hired daily at Fibonacci-scaled cost (the 15th
hire of a day already costs 610); one extra sheep's lifetime WOOL revenue
does not clear its own re-hire cost in any part of the season.

The actual fix came from a freshly-added reference notebook,
model_score/v16-rc5-high-score-8c-4s-premium-market-lead.ipynb: a
majority-consensus reconstruction of Nikita Lugovoy's (real, currently
top-10) 8 COW / 4 SHEEP route from three of his public replays, plus a
"premium market lead" overlay that moves a scheduled SELL one turn earlier
when the current turn has no town-shop demand for that item (queue
position at a shared market matters -- selling into the turn just before
demand lands avoids ceding that turn's price to whoever else moves first).
Independently re-verified here with this project's own benchmark.py
(not just the notebook's self-reported numbers) before adopting anything:

  - vs the actual shipped 6.3 (git commit 235c122, not the locally-dirty
    copy that was floating around from an unrelated concurrent session):
    20/20 wins, every seed, both seats, +4,209/game.
  - vs main_inspo (5.9): 10/10, +28,054/game (6.3 itself only reached
    +26,200/game against the same opponent).
  - vs models/5_6.py: 10/10, +33,128/game (6.3: +32,213/game).
  - vs models/4_5.py: 10/10, +17,565/game (6.3: +14,924/game).

Tried and rejected before adopting this route: grafting 6.3's entire
overlay stack (price_floor_guard, rank_sell_slots, premium_shift,
fertilizer_relay, opportunistic_sell, terminal_liquidation) on top of this
route -- statistically a coin flip against the route's own native overlay
(10/20 wins, -179/game, noise). None of those overlays add anything here;
this route's own scheduling already avoids the specific gaps they were
tuned to patch on our old, weaker backbone.

New in 6.4, the one addition that did measurably help: the notebook's own
premium market-lead overlay only front-runs 4 items (MELON, MILK,
STRAWBERRY, WOOL). Widening `_FR_ITEMS` to all 9 sellable items (adding
WHEAT, FERTILIZER, EGG, CARROT, TOMATO) improved every non-mirror
benchmark (main_inspo +28,054->+29,788, models/5_6 +33,128->+34,428,
models/4_5 unchanged within noise, 6.3 unchanged within noise) and won
17/20 in a direct mirror match against the unwidened version -- a real,
if modest, additive edge with no observed regression anywhere tested.

Everything else (the reconstructed route itself, weed repair, the repay/
front-run state machine) is unchanged from the source notebook's own
validated V16-RC5, checksum-verified against its own sha256 before use.
"""
import base64
import copy
import json
...
__version__ = "mapleleaf-6.4-nikita-8c4s-widened-premium-lead"
