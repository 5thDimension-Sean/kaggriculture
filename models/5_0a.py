"""Kaggriculture agent — MapleLeaf 5.0a
Route:   ep=90794783 P1 (same as 4.7) + 5.0 market improvements (3-step lookahead, overflow, cash exploit)
         Fresh route replaces stale ep=90794783 used in 4.7–4.9
Market:  price-impact SELL sort + NPC-demand persistence weighting
         + opponent-weighted impact sort (contested items sell first)
         + premium-shift 3-step lookahead [NEW: +3 at qty//4 added to 4.7's 2-step]
         ...
         + price-gate: hold sells when price < 20% of base (4.7 threshold kept)
         + overflow-sells: force-drain shed > 75 units (safety net for new route) [NEW]
         + opponent cash-state exploitation: expand sells when opp money < $1200 [NEW]
Safety:  shed-projection clamp so SELL quantities never exceed actual inventory

vs 4.7: new route from training-data-v3 top corpus; 3-step premium lookahead;
        overflow-sells wired in; opponent cash exploitation.
"""
import base64
import copy
import json
...
def _premium_shift(obs, action, step, thresh=8):
    """Advance-sell premium items up to 3 steps early when farms are converged.

    step+1 tranche: qty // 2  (half the planned sell, one step ahead)
    step+2 tranche: qty // 3  (a third, two steps ahead)
    step+3 tranche: qty // 4  (a quarter, three steps ahead) [NEW in 5.0]
    """
    ...
    market  = list(action.get("market") or [])
    prices  = _get(_get(obs, "market", {}) or {}, "prices", {}) or {}
    for offset, qty_div in ((1, 2), (2, 3), (3, 4)):
...
def _overflow_sells(obs, action):
    """When shed is near capacity, sell most-plentiful items not already being sold."""
    private = _get(obs, "private", {}) or {}
...
def _opponent_money(obs):
    """Get opponent's current bank balance."""
    seat  = _seat(obs)
    farms = list(_get(obs, "farms", []) or [])
    opp   = farms[1 - seat] if len(farms) >= 2 else {}
    return float(_get(opp, "money", 1e9) or 1e9)


def _cash_exploit_sells(obs, action, exposure):
    """When opponent is cash-poor, pre-sell their high-exposure no-recovery items.

    A cash-poor opponent cannot defend market prices (< $1200 means they can't
    buy product back to prop up prices). We dump no-recovery items that also
    happen to be the opponent's main production, keeping prices suppressed
    while they lack the capital to recover.  Only fires mid-game (200-690).
    """
    if _opponent_money(obs) > 1200:
        return action
    step = min(max(0, int(_get(obs, "step", 0) or 0)), len(_ACTIONS) - 1)
    if not (200 <= step < len(_ACTIONS) - 30):
        return action
    shed    = _projected_shed(obs, action)
    action  = _copy_action(action)
    market  = list(action.get("market") or [])
    ...
    for item in _NO_RECOVERY_ITEMS:
        if item in current or len(market) >= 10:
            continue
        if exposure.get(item, 0.0) < 0.8:
            continue
        qty = max(0, int(shed.get(item, 0) or 0))
        if qty >= 4:
            candidates.append((exposure.get(item, 0.0) * qty, item, qty))
    candidates.sort(reverse=True)
    for _, item, qty in candidates[:2]:
        if len(market) >= 10:
            break
        market.append(["SELL", item, qty])
        current.add(item)
...
def agent(obs):
    try:
        ...
        action   = _cash_exploit_sells(obs, action, exposure)
        action   = _safe_market(obs, action)
