"""MapleLeaf 5.3 — dual-route adaptive controller for Kaggriculture.
...
Runtime overlays on top of the route:
  ...
  3. Rank sell slots      — reorder SELLs by descending price-impact × demand-urgency
  4. Preempt shift        — move premium sells 1-3 turns earlier vs near-clone opponents
  5. Terminal liquidation — sell all shed inventory in the final 4 steps
  6. Price-floor guard    — skip route SELLs priced at $1 floor (market fully saturated)
...
Position-aware adjustments:
  ...
  - P0 uses the standard parameters.
"""
import base64
import copy
import json
...
__version__ = "mapleleaf-5.3-dual-route"
