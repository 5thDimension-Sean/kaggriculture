...
Runtime overlays on top of 5.3 routes:
  1. Weed repair          — DIG before BUILD/PLANT on weed tiles
  2. Repay preemption     — undo shifted sells on their original due step
  ...
  7. Wheat-arbitrage counter  — demotes WHEAT sells when market shows heavy wheat
     activity (inv > 10,200 or price < $23 at step ≥ 192), promoting premiums. [NEW]
...
_PREEMPT_MAX_CLONE_DISTANCE_P1 = 4   # P1 is more conservative: market reflects P0's sells
...
# Wheat-arbitrage counter thresholds (Improvement #1)
_WHEAT_ARB_STEP      = 192     # Start checking after day 8
_WHEAT_ARB_INV_HIGH  = 10200   # WHEAT inventory above equilibrium → heavy selling
_WHEAT_ARB_PRICE_LOW = 23      # WHEAT price below base $25 → market surplus
_WHEAT_ARB_MULTIPLIER     = 0.30   # Demote WHEAT score when arb detected

# Opponent cash exploit thresholds (Improvement #2)
_CASH_EXPLOIT_THRESHOLD   = 500    # Opponent money below this → they're constrained
_CASH_EXPLOIT_MULTIPLIER  = 1.20   # Boost premium-sell score when cash exploit active
...
def _order_score(obs, configuration, order, ctx=None):
    ...
    if ctx:
        # Wheat-arbitrage counter: demote WHEAT when market shows heavy wheat activity
...
def _compute_context(obs, step):
    """Build a context dict with game-state flags used by market overlays."""
    seat   = _seat(obs)
    opp    = 1 - seat
    market = _get(obs, "market", {}) or {}
    prices = _get(market, "prices", {}) or {}
